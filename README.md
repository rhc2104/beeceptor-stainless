...
to add `RONNIE_BEECEPTOR_API_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncRonnieBeeceptorAPI` instead of `RonnieBeeceptorAPI` and use `await` with each API call:
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `ronnie_beeceptor_api.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `ronnie_beeceptor_api.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `ronnie_beeceptor_api.APIError`.

```python
import ronnie_beeceptor_api
from ronnie_beeceptor_api import RonnieBeeceptorAPI
...
try:
    client.users.list()
except ronnie_beeceptor_api.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except ronnie_beeceptor_api.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except ronnie_beeceptor_api.APIStatusError as e:
...
You can enable logging by setting the environment variable `RONNIE_BEECEPTOR_API_LOG` to `info`.

```shell
$ export RONNIE_BEECEPTOR_API_LOG=info
```
...
```python
import httpx
from ronnie_beeceptor_api import RonnieBeeceptorAPI, DefaultHttpxClient

client = RonnieBeeceptorAPI(
    # Or use the `RONNIE_BEECEPTOR_API_BASE_URL` env var
...
with RonnieBeeceptorAPI() as client:
  # make requests here
  ...
...
```py
import ronnie_beeceptor_api
print(ronnie_beeceptor_api.__version__)
```
